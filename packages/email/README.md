...
Envoi d'emails transactionnels EduQuiz : vérification d'adresse, bienvenue,
reset password (à venir 1.5). Wrapper minimaliste autour de `nodemailer` +
...
Tous les templates sont des fonctions pures `(input) => EmailMessage`. Pour
l'envoi, passer le résultat à `sendEmail()`.
...
`docker-compose.dev.yml` lance MailHog. Tous les emails envoyés depuis les apps
en dev pointent dessus :
