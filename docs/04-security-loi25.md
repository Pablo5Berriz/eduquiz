...
Récapitulatif des contrôles déjà livrés au tag `v0.1.0` (avril 2026, Phase 1) :
...
| Sessions JWT révoquées côté serveur par version                                               | ✅ avec nuance | `@eduquiz/auth/config.ts` (`strategy=jwt`) + `User.sessionVersion` vérifié dans les helpers serveur                        |
...
| Invalidation des anciens JWT au reset password / changement mdp                               | ✅             | `forgot-password.ts` et `account.ts` incrémentent `User.sessionVersion`; `DELETE sessions` reste une défense en profondeur |
...
| Validation env stricte au boot (AUTH*\*, SMTP*\*, REDIS_URL)                                  | ✅             | Schémas Zod dans chaque paquet                                                                                             |
...
- Export en arrière-plan (V1 = synchrone, à passer en background si la taille
  des données devient problématique)
- Export PDF/ZIP Loi 25 non livré.
- Flux parent/mineur complet et double opt-in parental non livrés.
- EFVP et validation juridique formelle à produire.
- Inscription parent et mineur (Lots 10/11) avec consentement parental
  vérifiable
- Notifications, paramètres confidentialité fine, accessibilité (écrans 34/35/36
  — Lots ultérieurs)
