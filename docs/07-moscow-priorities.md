...
La priorisation vise un **premier lancement utilisable** permettant de tester le marché, puis une extension progressive. MoSCoW répond à la question : « si je devais couper, que garderais-je ? ».

## MUST — Indispensable au lancement V1

Sans ces éléments, le produit n'est ni légal, ni utile, ni testable sur le marché.
...
Fortement recommandé pour un produit complet, mais le produit peut vivre sans à très court terme.
...
Lots 0 → 1 → 2 → 10 → 13 (minimal) → 3 → 5 → 6 → 7 (partiel) → pages légales + export données.

Cela donne un produit utilisable par des adultes et des mineurs accompagnés, avec un catalogue sur 1 ou 2 matières, conforme à la Loi 25.

**V1.5** — ajout des Should prioritaires : Lots 4, 8, 9, 11, 15 (accessibilité).

**V2** — paiement complet (Lot 12), polish, PWA, Could.

**V3 (Partie 2)** — extension B2B école, développée dans une itération ultérieure.